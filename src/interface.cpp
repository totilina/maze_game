...
void Dem_des(){
	...
	// settextstyle(40, 0, "宋体");
	const char *text1 = "出于加深对DFS算法的理解，添加了这么一个模式，在这个模式将生成该迷宫的算法可视化，希望这个模式可以给对这方面给感兴趣的人一点启发。\n接下来让我简单介绍一下这个模式，刚进入这个模式会展现出这个迷宫最原始的样子(基本全是黑黑的墙)，而当你按下L键，该算法便会向下执行一步，绿色的方块是栈的节点，即遍历起点到红色节点中途需经过的节点，找到迷宫入口后将所有节点连起来便是通关路径，而红色方块是当前遍历位置，迷宫生成好后也可以继续玩游戏，生成中途按Esc键可以中止生成，再按一次Esc键可以返回主菜单\n注：只有坐标均为奇数的点才是节点，两节点间是被打通的墙，即两节点间会有一条路";
